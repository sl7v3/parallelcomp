...
void init_array_by_rank (int rank, int size){
  ...
  if (ROWS>COLS){
    ...
    double arr[proc_rows][proc_cols];
    int startCol; 
    int boundaryCol; 
    if (rank < size -1){
    ...
    for (int row = 0; row < proc_rows; row++){
      startCol  = rank*rowColMaxPerProc; 
      colSpan = boundaryCol - startCol;
      for (int col = 0; col < colSpan; col++){
        arr[row][col] = row*sin(row) + startCol*cos(startCol) + sqrt(row+startCol);
       //arr[row][col] = sqrt(row+startCol);
        startCol++;
      }
    }
   ...
   for (int row = 0; row < proc_rows; row++){
    ...
    int rowSpan;
    startRow  = rank*rowColMaxPerProc;
    rowSpan = boundaryRow - startRow;
    for (int row = 0; row < rowSpan; row++){
      for (int col = 0; col < proc_cols; col++){
        arr[row][col] = startRow*sin(startRow) + col*cos(col) + sqrt(startRow+col);
        //arr[row][col] = sqrt(startRow+col);
      }
      startRow++;
    }
    cout << "The array generated by " << rank << "th processor is below" <<endl;
