...
int i, length;
  for (length =1; length<= 101; length +=10){
    start = MPI_Wtime();
    for (i = 1; i<=10; i++){
      for (int snd_rank = 1; snd_rank< size; snd_rank++){
        MPI_Send(buffer, length, MPI_INT, snd_rank, ping, MPI_COMM_WORLD);
      }
      for (int rcv_rank = 1; rcv_rank< size; rcv_rank++){
        MPI_Recv(buffer, length, MPI_INT, rcv_rank, pong, MPI_COMM_WORLD, &status);     }
    }
    finish = MPI_Wtime();
    time = finish - start;
