...
MPI_Type_vector(length, COLS, 0, MPI_INT, &newType);MPI_Type_commit (&newType);
MPI_Send(&(my_ary[0][0]), length, newType, rank+1, ping, MPI_COMM_WORLD);
MPI_Recv(&(my_ary[0][0]), length, newType, rank+1, pong, MPI_COMM_WORLD, &status);
...
MPI_Type_vector(length, COLS, 0, MPI_INT, &newType);MPI_Type_commit (&newType);
MPI_Status status;
MPI_Send(&(my_ary[4][0]), length, newType, rank-1, ping, MPI_COMM_WORLD);
MPI_Recv(&(my_ary[3][0]), length, newType, rank-1, pong, MPI_COMM_WORLD, &status);
cout<< "Final array for " << rank << "th processor is below "<<endl;
print_ary(my_ary);
}
