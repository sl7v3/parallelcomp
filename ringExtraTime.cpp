...
int i, length = 1;
  ...
  for (int sends =0; sends< size; sends++){
    MPI_Bsend(buffer, length, MPI_INT, rank+1, ping, MPI_COMM_WORLD);
    MPI_Recv(buffer, length, MPI_INT, size-1, pong, MPI_COMM_WORLD, &status);
    sum+= buffer[0];
  }
  finish = MPI_Wtime();
  time = (finish - start)/size;
  cout << "Sum of all ranks received by processor at rank " <<rank<< " is "  << sum <<". This process took " << time << " seconds." << endl;
...
start = MPI_Wtime();  
  for (int sends =0; sends< size; sends++){
    MPI_Bsend(buffer, length, MPI_INT, 0, ping, MPI_COMM_WORLD);
    MPI_Recv(buffer, length, MPI_INT, rank-1, pong, MPI_COMM_WORLD, &status);
    sum+= buffer[0];
  }
  finish = MPI_Wtime();
